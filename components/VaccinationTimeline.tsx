...
export default function VaccinationTimeline() {
  ...
  const sortedVaccinations = [...vaccinations].sort((a, b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime())

  return (
    <div className="glass p-6 rounded-lg">
      <h2 className="text-2xl font-semibold mb-4 text-emerald-400 neon">Vaccination Timeline</h2>
      <ul className="space-y-4 mb-4">
        {sortedVaccinations.map((vax) => (
          <li key={vax.id} className="flex items-center justify-between bg-zinc-800 p-3 rounded-md">
            <div>
